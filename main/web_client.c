...
#if ESP_IDF_VERSION_MAJOR == 5 && ESP_IDF_VERSION_MINOR == 0				// Verifica a versao atual.
#include "driver/adc.h" 													// Necessario para o ADC1_GPIOxx_CHANNEL.
#endif
#include "esp_adc/adc_cali.h"												// Calibracao.
#include "esp_adc/adc_cali_scheme.h"										// Esquemas para calibracao.
#include "esp_adc/adc_oneshot.h"											// Modo de um ciclo por vez.

#include "cJSON.h"															// JSON.
#include "websocket_server.h"												// Servidor WebSocket.

static const char *TAG = "web_client";
extern MessageBufferHandle_t xMessageBufferToClient;

static void timer_cb(TimerHandle_t xTimer)									// Subrotina do WebSocket: 'Timer callback'.
{
	TickType_t nowTick;														// Cria a instancia do Timer.
	nowTick = xTaskGetTickCount();											// Salva o valor do 'Tick'.
	ESP_LOGD(TAG, "O Timer foi solicitado, agora=%"PRIu32, nowTick);		// Apenas debug.
	cJSON *request;															// Cria a requisicao.
	request = cJSON_CreateObject();											// Salva o retorno do Objeto requisitado.
	cJSON_AddStringToObject(request, "id", "timer-request");				// Adiciona string ao JSON.
	char *my_json_string = cJSON_Print(request);							// Cria string para recuperar o conteudo da requisicao.
	ESP_LOGD(TAG, "String JSON:\n%s",my_json_string);						// Apenas debug.
	size_t xBytesSent = xMessageBufferSendFromISR(xMessageBufferToClient, my_json_string, strlen(my_json_string), NULL);
	if (xBytesSent != strlen(my_json_string)) {ESP_LOGE(TAG, "String JSON: Falhou.");}
	cJSON_Delete(request);													// Finaliza a requisicao JSON.
	cJSON_free(my_json_string);												// Libera o recurso da string.
}

adc_channel_t gpio2adc(int gpio) 											// Converte de GPIO para ADC_CHANNEL (Compatibilidade entre os modelos ESP).
{
...
static bool adc_calibration_init(adc_unit_t unit, adc_atten_t atten, adc_cali_handle_t *out_handle)
{
	...
	if (!calibrated) 
	{
		ESP_LOGI(TAG, "Versao do esquema de calibracao: %s", "Curve Fitting");
	...
	if (!calibrated) 
	{
		ESP_LOGI(TAG, "Versao do esquema de calibracao: %s", "Line Fitting");
	...
	*out_handle = handle;
	if (ret == ESP_OK) {ESP_LOGI(TAG, "Calibrado com Sucesso.");} 
	else if (ret == ESP_ERR_NOT_SUPPORTED || !calibrated) {ESP_LOGW(TAG, "eFuse nao marcado, pulando a calibaracao por software.");} 
	else {ESP_LOGE(TAG, "Argumento invalido ou sem memoria.");}
	return (calibrated);
}
